...
const generateQuestionJson = async (document: any, templateNumber: number): Promise<object | void> => {
    ...
    try {
        ...
        // Generate question
        var questionGen = document.questionTemplate.replace(/\$\$\$/g, res[randomIndexes[0]].templateLabel);
...
const generateQuestionsArray = async (randomQuestionsTemplates: any): Promise<object[]> => {
    ...
    // and its answers
    const promises = randomQuestionsTemplates.map(async (template: any, i: number) => {
        try {
            const res = await generateQuestionJson(template, i);
            addObjectResponse(res);
        } catch (error) {
            console.error("Error while generating question for template: " + template)
            throw error
        }
    });
